...
function generateCitySkyline() {
    ...
    while (currentX < WIDTH) {
        const width = randomInt(35, 65);
        const height = randomInt(120, 320);
...
class Snake {
    ...
    checkCollision() {
        ...
        for (let i = 1; i < this.body.length; i++) {
            if (
                head[0] === this.body[i][0] &&
                head[1] === this.body[i][1]
            ) {
                return true;
            }
        }
    ...
    draw() {
        for (let i = 1; i < this.body.length; i++) {
            const px = this.body[i][0] * GRID_SIZE;
            const py = this.body[i][1] * GRID_SIZE;

            ctx.fillStyle = "rgba(255,35,75,0.25)";
            ...
            ctx.fillStyle = "rgba(255,35,75,0.86)";
...
function drawEyes(px, py, direction) {
    ...
    if (direction[0] === 1) {
        ellipse(cx + 5, cy - 5, 6, 3);
        ellipse(cx + 5, cy + 3, 6, 3);
    } else if (direction[0] === -1) {
        ellipse(cx - 5, cy - 5, 6, 3);
        ellipse(cx - 5, cy + 3, 6, 3);
    } else if (direction[1] === -1) {
        ellipse(cx - 5, cy - 5, 3, 6);
        ellipse(cx + 4, cy - 5, 3, 6);
    } else {
        ellipse(cx - 5, cy + 4, 3, 6);
        ellipse(cx + 4, cy + 4, 3, 6);
    }
}
...
class Food {
    ...
    respawn(snakeBody) {
        while (true) {
            const x = randomInt(0, GRID_WIDTH - 1);
            const y = randomInt(0, GRID_HEIGHT - 1);
            ...
            if (
                ...
                this.pos = [x, y];
                this.isSpecial = Math.random() < 0.25;
                break;
            }
        }
    }
    ...
    draw() {
        ...
        const cx =
            this.pos[0] * GRID_SIZE +
            GRID_SIZE / 2;

        const cy =
            this.pos[1] * GRID_SIZE +
            GRID_SIZE / 2;
...
function handleMove() {
    ...
    if (
        snake.body[0][0] === food.pos[0] &&
        snake.body[0][1] === food.pos[1]
    ) {
        const foodX =
            food.pos[0] * GRID_SIZE +
            GRID_SIZE / 2;

        const foodY =
            food.pos[1] * GRID_SIZE +
            GRID_SIZE / 2;
